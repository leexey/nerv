...
public:
    ...
    c_user_cmd* get_user_cmd(void* local_controller)
    {
        ...
        static auto GetCommandIndex = reinterpret_cast<void* (__fastcall*)(void*, int*)>(g_opcodes->scan(g_modules->m_modules.client_dll.get_name(), "48 83 EC 08 4C 8B 0D ? ? ? ? 4C 8B DA 48 8B 49 10 49 83 C1 10 48 85 C9 75 0E C7 02 ? ? ? ? 48"));

        if (!GetCommandIndex)
            return nullptr;
        int index = 0;
        GetCommandIndex(local_controller, &index);
        int commandIndex = index - 1;

        if (commandIndex == -1)
            commandIndex = 0xFFFFFFFF;

        static auto GetUserCmdBase = reinterpret_cast<void* (__fastcall*)(void*, int)>(g_opcodes->scan(g_modules->m_modules.client_dll.get_name(), "48 89 4C 24 ? 41 56 41 57 48 83 EC 48 4C 63 FA 4C 8B F1 4C 89 7C 24 ? 4A 8B 04 F9"));
        if (!GetUserCmdBase)
            return nullptr;

        static auto off_18203A7F8 = *reinterpret_cast<void**>(g_opcodes->scan_absolute(g_modules->m_modules.client_dll.get_name(), "48 8B 0D ? ? ? ? E8 ? ? ? ? 48 8B CF 4C 8B F8 44 8B B0", 0x3));

        if (!off_18203A7F8)
            return nullptr;

        void* pUserCmdBase = GetUserCmdBase(off_18203A7F8, commandIndex);
        if (!pUserCmdBase)
            return nullptr;

        DWORD sequenceNumber = *reinterpret_cast<DWORD*>((uintptr_t)pUserCmdBase + 0x5910);
        static auto GetUserCmd = reinterpret_cast<c_user_cmd * (__fastcall*)(void*, DWORD)>(g_opcodes->scan(g_modules->m_modules.client_dll.get_name(), "40 53 48 83 EC 20 8B DA E8 ? ? ? ? 4C 8B C0 85 DB 7F 0A 33 C9 8B C1"));

        if (!GetUserCmd)
            return nullptr;

        c_user_cmd* pUserCmd = GetUserCmd(local_controller, sequenceNumber);
        if (!pUserCmd)
            return nullptr;
